  ...
  try {
    ...
    if (["admin", "instructor"].includes(user.role)) {
      users = user.role === "admin"
        ? await rows(`SELECT u.id,u.email,u.name,u.role,u.status,u.phone,u.whatsapp,u.country,u.city,u.specialty,u.college_id AS "collegeId",u.university_id AS "universityId",u.year_id AS "yearId",u.trusted_device_id AS "trustedDeviceId",u.created_at AS "createdAt" FROM users u ORDER BY u.id DESC`)
        : await rows(`SELECT DISTINCT u.id,u.email,u.name,u.role,u.status,u.phone,u.whatsapp,u.country,u.city,u.specialty,u.college_id AS "collegeId",u.university_id AS "universityId",u.year_id AS "yearId",u.created_at AS "createdAt" FROM users u JOIN enrollments e ON e.user_email=u.email JOIN courses c ON c.id=e.course_id WHERE c.instructor_email=$1 AND u.role='student' ORDER BY u.id DESC`, [user.email]);
...
export async function POST(request: Request) {
  try {
    ...
    if (action === "profile") {
      ...
      await pool.query(`UPDATE users SET name=$1,phone=$2,whatsapp=$3,country=$4,city=$5,specialty=$6,college_id=$7,university_id=$8,year_id=$9,status=CASE WHEN status IN ('rejected','needs_changes') THEN 'pending' ELSE status END,updated_at=NOW() WHERE email=$10`, [name, phone, clean(data.whatsapp, 30), clean(data.country, 80), clean(data.city, 80), clean(data.specialty, 160), collegeId, universityId, yearId, user.email]);
