...
export async function POST(request: Request) {
  try {
    ...
    const role = clean(body.role, 20) === "instructor" ? "instructor" : "student";
    ...
    let collegeId: number | null = null, universityId: number | null = null, yearId: number | null = null, level = "", stageValue = "";

    if (role === "student") {
      stageValue = stage;
      if (!Object.keys(stageLevels).includes(stageValue)) {
        return Response.json({ error: "Choose your education stage" }, { status: 400 });
      }
      if (stageValue === "university") {
      ...
      } else if (stageValue === "high_school") {
